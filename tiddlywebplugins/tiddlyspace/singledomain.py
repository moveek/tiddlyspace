...
class OutputSingleDomain(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if req_entity.find('/space/') != -1:
            return (self._handle_relative_link(environ, output) for
    ...
    def _handle_relative_link(self, environ, output):
        try:
            space = determine_space(environ, environ['SERVER_NAME'])
            output = re.sub(r'(href=./|src=./)', r'\1space' + '/' + space + '/', output)
